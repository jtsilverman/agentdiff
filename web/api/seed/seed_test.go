...
func TestSeed_PopulatesScenariosOnEmptyDB(t *testing.T) {
	...
	expectedNames := map[string]bool{
		"api-endpoint-rename":          true,
		"auth-migration-md5-to-bcrypt": true,
		"new-endpoint-with-tests":      true,
	}
	for _, b := range baselines {
		if !expectedNames[b.Name] {
			t.Errorf("baseline %q is not one of the 3 expected short engineering-style names (api-endpoint-rename / auth-migration-md5-to-bcrypt / new-endpoint-with-tests)", b.Name)
		}
		if b.TraceCount < 3 {
...
// TestSeed_TracesHaveKeyDecisionMetadata enforces that every seeded trace
// carries a non-empty `key_decision` metadata key. The redesigned trace-detail
// page renders this as a callout (.td-keydecision); without it the callout
// silently falls back to empty content.
func TestSeed_TracesHaveKeyDecisionMetadata(t *testing.T) {
	database := testDB(t)
	...
	for _, b := range baselines {
		...
		for _, td := range traces {
			decision, ok := td.Metadata["key_decision"]
			if !ok || strings.TrimSpace(decision) == "" {
				t.Errorf("trace %q under baseline %q is missing non-empty metadata.key_decision", td.Name, b.Name)
			}
		}
	}
}
