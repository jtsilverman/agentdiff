...
func TestLoadCachesFromJSON_RoundTripsTriageEntry(t *testing.T) {
	...
	cacheJSON := []byte(`{
		"triage": [{
			"trace_a_name": "seed-tool-order-stable/run-1",
			"trace_b_name": "seed-tool-order-stable/run-2",
	...
	traceA := findTraceByName(t, database, "seed-tool-order-stable/run-1")
	traceB := findTraceByName(t, database, "seed-tool-order-stable/run-2")
...
func TestLoadCachesFromJSON_RoundTripsTranscriptEntry(t *testing.T) {
	...
	cacheJSON := []byte(`{
		"triage": [],
		"transcript": [{
			"trace_name": "seed-tool-order-stable/run-1",
	...
	trace := findTraceByName(t, database, "seed-tool-order-stable/run-1")
