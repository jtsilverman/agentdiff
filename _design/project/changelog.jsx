/* changelog.jsx — reverse-chronological shipped-features log
   Plain English, no jargon. "Step-by-step comparison" not "edit-distance alignment".
*/

const { useState: useStateCL, useMemo: useMemoCL } = React;

// Real entries from the git log — verbatim, do not invent.
const ENTRIES = [
  // ── May 2026 ──
  { date: "2026-05-27", tag: "feature", title: "Site redesign",
  ...
  { date: "2026-05-27", tag: "feature", title: "Realistic seed scenarios",
  ...
  { date: "2026-05-27", tag: "feature", title: "Trace detail rebuild",
  ...
  { date: "2026-05-27", tag: "feature", title: "Diff page rebuild",
  ...
  { date: "2026-05-27", tag: "docs", title: "Developer reference",
  ...
  { date: "2026-05-20", tag: "feature", title: "Similar-traces panel",
    body: "\"Find traces like this one\" on the trace detail page, backed by Voyage embeddings and cosine similarity." },
  { date: "2026-05-19", tag: "feature", title: "GitHub Action with inline screenshots",
  ...
  { date: "2026-05-19", tag: "feature", title: "Inline prompt editor",
  ...
  { date: "2026-05-19", tag: "feature", title: "Replay scrubber",
  ...
  { date: "2026-05-19", tag: "feature", title: "Cost/latency heatmap on the path graph",
    body: "Each tool-call node shaded by token cost and wall time so expensive or slow steps stand out at a glance." },
  { date: "2026-05-18", tag: "feature", title: "Counterfactual replay",
  ...
  { date: "2026-05-18", tag: "feature", title: "Audit-to-test",
  ...
  { date: "2026-05-18", tag: "feature", title: "AI-written transcript summaries",
  ...
  { date: "2026-05-18", tag: "feature", title: "First-boot demo gallery",
    body: "Out-of-the-box scenarios load on first run so visitors see real comparisons without any setup." },
  { date: "2026-05-18", tag: "feature", title: "AI-written explanations for divergent runs",
  ...
  { date: "2026-05-18", tag: "feature", title: "Path-graph view",
...
function monthGroup(iso) {
  const d = new Date(iso + "T12:00:00Z");
  return d.toLocaleString("en-US", { month: "long", year: "numeric", timeZone: "UTC" });
}

// ─── Tag filter ───
const TAGS = ["all", "feature", "fix", "docs", "infra"];
  ...
  <article className={"cl-entry tag-" + e.tag} data-tag={e.tag}>
    ...
    <div className="cl-entry-body">
      <header className="cl-entry-head">
        ...
        <span className={"badge " + TAG_KIND[e.tag] + " mono cl-entry-tag"}>{TAG_LABELS[e.tag]}</span>
      </header>
...
// ─── Main ───
const Changelog = () => {
  const [tagFilter, setTagFilter] = useStateCL("all");
  ...
  // Group by month — keep order from ENTRIES (already reverse-chrono)
  const groups = useMemoCL(() => {
    const map = new Map();
    filtered.forEach(e => {
  ...
  // Per-tag counts for the segmented bar
  const counts = useMemoCL(() => {
    const c = { all: ENTRIES.length, feature: 0, fix: 0, docs: 0, infra: 0 };
    ENTRIES.forEach(e => { c[e.tag] = (c[e.tag] || 0) + 1; });
    return c;
  }, []);
  ...
  return (
    <>
      ...
      <main>
        {/* Hero */}
        <header className="tr-hero cl-hero" data-screen-label="01 Changelog hero">
            ...
            <div className="cl-hero-grid">
              <div>
                ...
                <p className="tr-hero-sub">
                  Everything that's shipped, newest first. Each entry is one or two plain-English sentences about what changed for you.
                </p>
              </div>
              ...
              <aside className="cl-hero-stats tr-hero-stats">
                ...
                <div className="bl-stat">
                  ...
                  <span className="bl-stat-val" style={{color: "var(--accent)"}}>{counts.feature}</span>
                </div>
                <div className="bl-stat">
                ...
                <div className="bl-stat">
                  ...
                  <span className="bl-stat-val" style={{color: "var(--novel)"}}>{counts.docs}</span>
                </div>
              </aside>
            </div>
          </div>
        ...
        <div className="container">
          ...
          <div className="cl-controls">
            ...
            <div className="seg">
              {TAGS.map(t => (
                ...
                >
                  {t}
                  {counts[t] !== undefined && <span className="cl-seg-count mono">{counts[t]}</span>}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="cl-timeline">
            {groups.length === 0 && (
              <div className="tr-empty">
                ...
                <button className="btn" onClick={() => setTagFilter("all")}>Show all</button>
              </div>
            )}
            {groups.map(([month, entries]) => (
              <section key={month} className="cl-month">
                <header className="cl-month-head">
                  ...
                  <span className="mono dim cl-month-count">{entries.length} {entries.length === 1 ? "entry" : "entries"}</span>
                </header>
                <div className="cl-entries">
                  {entries.map((e, i) => <Entry key={month + i} e={e}/>)}
                </div>
              </section>
            ))}
          </div>
          ...
          <footer className="dx-foot">
            ...
            <a href="docs.html" className="btn">
              <Icon.ArrowRight style={{transform: "rotate(180deg)"}}/> Reference
            </a>
          </footer>
        </div>
      </main>
