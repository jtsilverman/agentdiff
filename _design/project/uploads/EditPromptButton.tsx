...
import type { Step, EditPromptResponse } from '@/lib/types';
...
export default function EditPromptButton({ traceId, steps }: EditPromptButtonProps) {
  ...
  const handleSubmit = async () => {
    const text = modifiedPrompt.trim();
    if (!text) return;
    setSubmitting(true);
    setError(null);
    setResult(null);
    try {
      const res = await editPrompt(traceId, stepIndex, text);
      setResult(res);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'failed to run prompt edit');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="flex flex-col gap-3 rounded border border-gray-800 bg-gray-900 p-3">
      <Text className="text-sm">Rewrite the user prompt at a chosen step and re-run the agent:</Text>
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end">
        <label className="flex flex-col text-xs text-gray-300">
          Step
          <select
            aria-label="Prompt-edit step"
            value={stepIndex}
        ...
        <div className="flex-1">
          <Textarea
            placeholder="Rewritten prompt for this step"
            value={modifiedPrompt}
            onValueChange={setModifiedPrompt}
            disabled={submitting}
            rows={3}
          />
        </div>
      ...
      <div className="flex items-center gap-2">
        <Button
          ...
          size="sm"
          disabled={submitting || !modifiedPrompt.trim()}
        >
          {submitting ? 'Running...' : 'Re-run from here'}
        </Button>
        <Button
          onClick={() => {
            setExpanded(false);
